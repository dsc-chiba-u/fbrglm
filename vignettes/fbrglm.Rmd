...
The current release runs under `infer = "none"`: it returns regularized
point estimates and **does not** report classical standard errors,
z values, p values, or confidence intervals — these are not valid
classical quantities for a penalized fit, so fbrglm suppresses them
by default. Honest post-selection inference (via data splitting or
selective inference) is on the roadmap; see the package `TODO.md`.

For two longer end-to-end walkthroughs, see the sibling vignettes:

- `vignette("fbrglm-real-data")` — a real-data pipeline on
  `MASS::birthwt` from read → split → fit → predict → evaluate →
  visualize → reuse.
- `vignette("fbrglm-migration")` — the same logistic model written
  three ways (`stats::glm`, raw `glmnet`, and `fbrglm`), with a
  side-by-side comparison table.
- `vignette("fbrglm-families")` — the full family catalog with a
...
**Supported (core)** — `gaussian`, `binomial`, `poisson`, arbitrary
`stats::family` objects (including `stats::Gamma`), and fixed-theta
negative binomial via `MASS::negative.binomial(theta = ...)`.

**Supported (experimental)** — native Cox (`family = "cox"` with a
`survival::Surv()` response), `multinomial`, and `mgaussian`. These
paths are exercised by the tests but not yet validated across the
full range of specialised uses (Cox strata / tie handling,
multinomial grouped / ungrouped, etc.); the family-catalog vignette
(`vignette("fbrglm-families")`) shows the working shape of each.

**Not implemented / out of scope**

- `infer = "split"` and `infer = "selective"` — planned; only
  `infer = "none"` is implemented in this release.
- Classical `glm()`-style standard errors, z / p values, and
  confidence intervals — intentionally suppressed under
  `infer = "none"` (regularized point estimates do not have a valid
  classical Wald sampling distribution).
- Joint theta estimation in the spirit of `MASS::glm.nb()` — pass a
  fixed theta to `MASS::negative.binomial()` directly.
- The `x` / `y` direct-matrix entry point is reserved but not yet
  wired; supply `formula` + `data` instead.
